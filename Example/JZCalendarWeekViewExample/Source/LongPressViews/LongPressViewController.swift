...
extension LongPressViewController: JZLongPressViewDelegate, JZLongPressViewDataSource {
	func weekView(_ weekView: JZLongPressWeekView, editingEvent: JZBaseEvent, didEndMoveLongPressAt startDate: Date, didUpdateDuration newDuration: Int, pendingEventView: EventView) {
		...
		viewModel.events[selectedIndex].endDate = startDate.add(component: .minute, value: newDuration)
	...
	func weekView(_ weekView: JZLongPressWeekView, editingEvent: JZBaseEvent, didEndMoveLongPressAt startDate: Date, pendingEventView: EventView) {
		...
		DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
			guard let cell = weekView.collectionView.visibleCells.first(where: {($0 as! LongPressEventCell).event.id ==  event.id}) else {return}
			pendingEventView.frame.origin = cell.frame.origin
			pendingEventView.frame.size = cell.frame.size
		}
	}
