...
open class JZBaseWeekView: UIView {
    ...
    open func updateWeekView(to date: Date) {
        ...
        let start = getDateForPoint(.init(x: flowLayout.rowHeaderWidth + flowLayout.contentsMargin.left, y: 0))
        
        if start.compare(date) == date.compare(end) {
            let contentWidth = self.collectionView.contentSize.width - flowLayout.rowHeaderWidth - flowLayout.contentsMargin.left
            
            (0...(Int(contentWidth / flowLayout.sectionWidth))).forEach { section in
                if getDateForSection(section).startOfDay == date.startOfDay {
                    isMovingToNewDate = true
                    self.collectionView.setContentOffsetWithoutDelegate(CGPoint(x: (CGFloat(section) * flowLayout.sectionWidth),y: self.collectionView.contentOffset.y), animated: true)
                    //                    self.collectionView.setContentOffset(, animated: true)
                }
            }
        } else {
